...
import {Image as ImageRN, StyleSheet, Text, TouchableOpacity, View} from 'react-native';
...
export default function FindScanEucalyptusTreeScreen() {
  ...
  const findTreeText = "Go find and scan a eucalyptus tree to start the lesson.";
  const viewTreeText = "View eucalyptus tree photos";
  const incorrectTreeText = "This doesn’t seem like a eucalyptus tree. Take a closer look at the reference photos.";
  ...
  const [showSampleImageScreen, setShowSampleImageScreen] = useState<boolean>(false);
      ...
      <View style={StyleSheet.absoluteFill}>
        ...
        {!scanningStarted &&
          <View style={styles.messageHolder}>
            ...
            <TouchableOpacity onPress={() => {setShowSampleImageScreen(true)}}>
              <Text style={{color: 'background: rgba(70, 140, 247, 1)', fontSize: 16, marginTop: 24}}>{viewTreeText}</Text>
            </TouchableOpacity>
          </View>
        }
        { scanningStarted && imageClass && imageClass.indexOf('eucalyptus') == -1 &&
          <View style={styles.messageHolder}>
        ...
        { imageClass && imageClass.indexOf('eucalyptus') == 0 &&
          <View style={styles.successMessageHolder1}>
            <Text style={{color: 'white', fontSize: 16}}>{correctTreeText}</Text>
          </View>
        }
        { imageClass && imageClass.indexOf('eucalyptus') == 0 &&
          <View style={styles.successMessageHolder2}>
            <Text style={{color: 'white', fontSize: 16}}>{wouldYouLikeToLearnText}</Text>
          </View>
        }
      </View>
      {showSampleImageScreen &&
        <SampleEucalyptusTreesScreen />
      }
      {showSampleImageScreen &&
        <View style={styles.closeIcon}>
          <TouchableOpacity onPress={() => {setShowSampleImageScreen(false)}}>
            <ImageRN source={{uri: closeIcon}} style={{width: 40, height: 40}}/>
          </TouchableOpacity>
        </View>
      }
    </View>
