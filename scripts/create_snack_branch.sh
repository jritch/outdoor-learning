...
if [[ `git status --porcelain` ]]; then
  ...
  git add -A
  git commit -am "[AUTO GENERATED] Remove android & ios folders for snack export $TODAY"
  ...
  printf "\n"
  printf "\n"
  echo "✅ Pushed to branch: $REMOTE_URL/tree/$BRANCH"

  printf "\n"

  echo "➡️ Now go to https://snack.expo.dev, select the three dots next to Project on the left, and select Import git repository and use the link above."
